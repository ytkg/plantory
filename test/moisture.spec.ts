...
describe("moisture status calculation", () => {
  it("calculates linearly interpolated P5 and P95 ranges", () => {
    ...
    expect(toMoistureStatus(metric(1, "鉢", "weight", -10, 2, 38))).toEqual({ plant_id: 1, name: "鉢", moisture: 0 });
    expect(toMoistureStatus(metric(1, "鉢", "weight", 50, 2, 38))).toEqual({ plant_id: 1, name: "鉢", moisture: 100 });
  });
  ...
  it("inverts soil moisture and rounds the relative value", () => {
    ...
    ])).toEqual([{ plant_id: 1, name: "カランコエ", moisture: 67 }]);
  });
  ...
  it("falls back to weight, preserves duplicate names, and omits unavailable values", () => {
    expect(buildMoistureStatuses([
      ...
      { plant_id: 1, name: "同じ名前", moisture: 33 },
      { plant_id: 2, name: "同じ名前", moisture: 67 },
    ]);
  });
});

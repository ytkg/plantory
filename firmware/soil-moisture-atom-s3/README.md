...
起動時にWi-Fiへ接続して植物名とPlantoryが算出した相対水分量を取得し、画面に表示します。ADC値は送信時に1秒ごとに10回読み取り、その平均を`soil_moisture`として記録します。自動送信は毎日0時・6時・12時・18時で、ATOM S3のボタンを押して任意のタイミングでも送信できます。送信成功後は相対水分量を再取得します。

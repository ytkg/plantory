...
日本時間の0時・6時・12時・18時に自動送信します。起動直後に自動送信することはありません。ADC値は送信時に1秒ごとに10回読み取り、その平均を`soil_moisture`として記録します。ダブルタップで任意の即時送信、2秒長押しで最大5回のWi-Fi再接続を開始できます。シングルタップは何もしません。送信成功後は相対水分量を再取得します。

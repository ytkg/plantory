...
`firmware/`で`npm test`を実行すると、各機種の`metrics_schedule.h`をC++17コンパイラで検証します（依存パッケージ・実機・秘密情報は不要）。8種類の間隔、毎正時の取得、設定変更直後の送信、取得失敗、再起動、通信待ち、重複処理、年またぎを確認します。GitHub Actionsでも同じテストを実行します。

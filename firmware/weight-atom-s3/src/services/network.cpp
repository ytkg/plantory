...
bool connectWifi(WaitHandler onWait) {
  ...
  while (WiFi.status() != WL_CONNECTED && millis() - startedAt < config::WIFI_TIMEOUT_MS) { delay(250); if (onWait != nullptr) onWait(); }
  return isConnected();
}
bool isConnected() { return WiFi.status() == WL_CONNECTED; }
void beginOta() { if (!isConnected()) return; ArduinoOTA.setHostname(config::OTA_HOSTNAME); ArduinoOTA.begin(); }
...
ReconnectEvent updateReconnect() {
  ...
  if (isConnected()) { reconnectActive = false; return ReconnectEvent::Connected; }
  if (millis() - reconnectStartedAt < config::WIFI_TIMEOUT_MS) return ReconnectEvent::None;
  if (reconnectTry >= config::WIFI_RECONNECT_MAX_ATTEMPTS) { reconnectActive = false; return ReconnectEvent::Failed; }
  beginReconnectAttempt();
  return ReconnectEvent::Attempting;
}
bool reconnecting() { return reconnectActive; }

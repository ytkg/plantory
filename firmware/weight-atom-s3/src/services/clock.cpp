...
String nextSendText(const struct tm& current) {
  for (size_t index = 0; index < config::SEND_HOUR_COUNT; ++index) {
    ...
    if (current.tm_hour < hour || (current.tm_hour == hour && current.tm_min == 0 && current.tm_sec < 1)) { char buffer[9]; snprintf(buffer, sizeof(buffer), "%02d:00:00", hour); return String(buffer); }
  }
  return "00:00:00";
}
}  // namespace plantory::clock

...
- 一覧の水分量グラフは直近30件を表示し、記録日時に比例する連続した時間軸で実測点をつなぐ。tooltipには日本時間の記録日時と水分量を表示する。
...
- Markdownの階層は、日付、観察日記のセクション見出し、植物名、本文の順に視認性を下げる。日付は24px（広い画面では30px）、セクション見出しは20px（広い画面では24px）、植物名は18px（広い画面では20px）、本文は16px相当とし、植物名の太字を維持する。スマートフォンでも同じ階層を保つ。

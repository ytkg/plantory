...
- グラフはChart.js v4.5.1を使用する折れ線グラフで、各種類の直近30件を時系列順に描画する。通常時に数値ラベルは表示しない。PCではグラフ上へのホバー、タッチ端末ではタップでツールチップを表示し、日本時間の受信時刻、指標名、API値の精度を保った数値、単位を確認できる。日時が不正な場合は「日時不明」と表示する。複数系列のグラフでは同じ時刻の値をまとめて表示する。

...
- `soil-moisture-atom-s3` と `weight-atom-s3` は、起動直後と以降1時間ごとに公開API `GET /api/status` から設定済みの`PLANT_ID`の`recorded_at`を確認する。最終記録から6時間以上経過している場合だけ自動計測・送信する。取得に失敗した確認回では送信せず、次の1時間後に再確認する。端末側へ前回送信時刻は永続化しない。

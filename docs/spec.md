...
- 植物カードには、最新metricsの種類・値・日本時間の受信時刻を表示する。metricsがない場合は「まだ測定がありません」と表示する。
...
- 公開トップページで、最新の室温・湿度・CO₂濃度と最終取得時刻を、植物のようすと観察日記の間に表示する。
...
- 重量は数値を大きくして1秒ごとに表示する。桁数が多いときは画面内に収まる大きさに縮小する。±1.0gは0.0gとして扱う。日本時間0/6/12/18時とATOM S3画面の1回タップ時に100ms間隔で10回測定し、平均を `weight` として送信する。
- 画面を350ms以内に2回タップすると、Mini Scalesの現在の荷重をゼロ点として調整する。読み取り失敗や非有限値は送信しない。連続失敗時はI²Cを終了・再初期化して接続を試みる。起動時にゼロ点は変更しない。

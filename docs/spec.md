...
- 一覧の水分量グラフは直近30件を表示し、記録日時に比例する連続した時間軸で実測点をつなぐ。横軸の右端と目盛りはページ表示時の現在時刻を超えない。tooltipには日本時間の記録日時と水分量を表示する。
...
- グラフは選択期間内かつ現在時刻以前の全実測点を時刻に比例する連続軸で表示し、横軸の右端と目盛りは現在時刻を超えない。Chart.jsの単調補間で点をつなぐが、実測値・APIレスポンスに補間値を追加しない。縦軸は表示中の生値の範囲と余白に合わせ、0始まりや0〜100%固定にしない。1件だけは点として表示する。

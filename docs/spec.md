...
- `soil-moisture-atom-s3` と `weight-atom-s3` は日本時間の0時・3時・6時・9時・12時・15時・18時・21時に自動計測・送信する。各時刻の送信は1回だけとし、起動直後は自動送信しない。
...
- 日本時間の0時・3時・6時・9時・12時・15時・18時・21時に自動計測・送信する。各時刻の送信は1回だけとし、起動直後に自動送信はしない。BtnAの短押しはゼロ点調整、ダブルタップは即時送信、2秒長押しは最大5回のWi-Fi再接続である。再接続の試行中・成功・失敗は画面に表示し、失敗後は再度長押しでやり直せる。

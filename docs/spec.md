...
画面HTMLはWorker内でHono JSX（TSX）によりサーバー描画する。共通のレイアウト、ヘッダー、フッターは `src/components/layout.tsx`、各ページの構造は `src/components/pages.tsx` で管理する。全画面のナビゲーション、モバイルメニュー、ログアウト導線、フッターはこの部品を変更して更新する。ブラウザー側のJavaScript、CSS、Chart.jsなどの静的アセットは従来どおり配信する。

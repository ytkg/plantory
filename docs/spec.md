...
- 一覧の水分量グラフは直近30件を、計測データページと同じ白いカード、時刻に比例する連続軸・単調補間・実測点・日時と割合を示すtooltipで表示する。横軸の右端と目盛りはページ表示時の現在時刻を超えない。縦軸は0〜100%に固定する。
...
- グラフは一覧の水分量グラフと同じ白いカードと表示スタイルで、選択期間内かつ現在時刻以前の全実測点を時刻に比例する連続軸で表示し、横軸の右端と目盛りは現在時刻を超えない。Chart.jsの単調補間で点をつなぐが、実測値・APIレスポンスに補間値を追加しない。縦軸は表示中の生値の範囲と余白に合わせ、0始まりや0〜100%固定にしない。1件だけは点として表示する。

...
export const apiKeys = sqliteTable("api_keys", { id: integer().primaryKey(), name: text().notNull(), keyHash: text("key_hash").notNull().unique(), scope: text().notNull(), createdAt: text("created_at").notNull().default(sql`CURRENT_TIMESTAMP`), lastUsedAt: text("last_used_at"), revokedAt: text("revoked_at") });

...
plantRoutes.get("/:id/metrics", async (c) => {
  ...
  return Number.isSafeInteger(id) && id > 0 ? authenticated(c, "read", () => listMetrics(id, c)) : c.json({ error: "Plant not found." }, 404);
});
plantRoutes.post("/:id/metrics", async (c) => {
  const id = Number(c.req.param("id"));
  return Number.isSafeInteger(id) && id > 0 ? authenticated(c, "write", () => createMetric(id, c)) : c.json({ error: "Plant not found." }, 404);
});
plantRoutes.delete("/:id/metrics", async (c) => {
  const id = Number(c.req.param("id"));
  return Number.isSafeInteger(id) && id > 0 ? authenticated(c, "write", () => deleteMetrics(id, c)) : c.json({ error: "Plant not found." }, 404);
});
